...
export const ProfileHeader = ({ user, isEditing, formData, onFormDataChange }: ProfileHeaderProps) => {
  const [profilePictureUrl, setProfilePictureUrl] = useState<string | undefined>(user.avatar);
    ...
    const loadProfilePicture = async () => {
      if (!isEditing) {
        ...
        try {
          ...
          const pictureUrl = files.profilePicture?.optimized || 
  ...
  const handleProfilePictureUpload = (url: string, urlHD?: string, urlOptimized?: string) => {
    // Use optimized URL if available, fallback to HD, then original
